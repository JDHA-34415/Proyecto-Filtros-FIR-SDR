...
# 1. Parámetros según el documento [cite: 13-18, 33]
fs = 1e6            # Frecuencia de muestreo (1 MHz para cubrir señales de hasta 300kHz)
cutoff = 100e3      # Frecuencia de corte: 100 kHz [cite: 16]
numtaps = 32       # Orden: 32 taps [cite: 17]
window = 'hamming'  # Ventana: Hamming [cite: 18]
...
# 3. Calcular la respuesta en frecuencia [cite: 8]
w, h = signal.freqz(taps, worN=8000)
f = w * fs / (2 * np.pi) # Convertir radianes a Hz

# 4. Visualización [cite: 38, 68]
...
# Imprimir los coeficientes resultantes (opcional para el informe) [cite: 66]
